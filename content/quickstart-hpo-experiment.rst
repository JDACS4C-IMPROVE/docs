...
Your model must be CANDLE compliant, containerized, and packaged in a singularity image. You can identify the image file by the **\*.sif** suffix. Default definition files can be found in the `IMPROVE Singularity repository <https://github.com/JDACS4C-IMPROVE/Singularity>`_. Follow the [install instruction](https://example.com/quickstart-build-and-deploy-models) to deploy curated default models. The container should expose the following interface scripts:
